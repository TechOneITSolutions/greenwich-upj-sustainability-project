...
const Navbar = () => {
  ...
  return (
    <>
      {/* Top Info Bar */}
      <div className="top-0 z-[60] w-full bg-[#4aa537] text-white py-2 px- sm:px-6 lg:px-8">
            ...
            <div className="flex items-center">
              <Link href="/" className="flex-shrink-0 group">
                <div className="flex items-center gap-4">
                  <div className="flex">
                    <img src="/logo.png" alt="Greenwich" className="w-55 h-auto" />
                  </div>
                </div>
              </Link>
            </div>
              ...
              <div className="flex items-center space-x-2">
                {navLinks.map((link) => (
                  <Link 
                    key={link.name}
                    href={link.href} 
                    className="px-4 py-2 text-sm font-bold text-gray-700 hover:text-[#4aa537] transition-colors"
                  >
                    {link.name}
                  </Link>
                ))}
            ...
            <div className="px-4 pt-2 pb-6 space-y-1 sm:px-6">
              {navLinks.map((link) => (
                <Link 
                  key={link.name}
                  href={link.href} 
                  className="text-gray-700 hover:text-[#4aa537] block px-3 py-4 text-base font-bold border-b border-gray-50"
