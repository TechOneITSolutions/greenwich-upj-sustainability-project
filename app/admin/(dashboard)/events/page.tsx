...
export default async function EventsAdmin(props: {
  ...
  let dbQuery = supabase.from('events').select('*', { count: 'exact' })
  if (query) {
    dbQuery = dbQuery.or(`title.ilike.%${query}%,location.ilike.%${query}%`)
  }
  const { data: events, count } = await dbQuery.order('date', { ascending: false }).range(offset, offset + limit - 1)
    ...
    <div>
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        ...
        <div className="flex items-center gap-4 flex-wrap w-full sm:w-auto">
          <AdminTableFilter placeholder="Search events..." />
                  ...
                  <td colSpan={4} className="p-16 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500">
                        {query ? `No events matching "${query}"` : 'Get started by creating a new event.'}
                      </p>
                    </div>
                  </td>
                </tr>
